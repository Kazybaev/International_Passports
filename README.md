...
> Важно: репозиторий не должен содержать реальные паспорта, персональные данные, секреты Streamlit или скачанные веса моделей. Используйте только synthetic/consented тестовые изображения и храните тяжёлые артефакты вне Git.
...
Перед публикацией проверьте рабочее дерево:
...
Что уже настроено для репозитория:

- `.gitignore` исключает локальные окружения, секреты, веса моделей и новые benchmark-артефакты;
- `.dockerignore` не отправляет в Docker build context Git-метаданные, тестовые отчёты, модели и локальные файлы;
- GitHub Actions workflow запускает `pytest -q` на Python 3.11 для push в `main/master` и pull request.

Если в истории Git уже есть реальные документы или большие model-файлы, их нужно удалять из истории отдельной операцией перед публичной публикацией.
